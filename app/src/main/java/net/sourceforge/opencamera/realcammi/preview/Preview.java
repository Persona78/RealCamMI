package net.sourceforge.opencamera.realcammi.preview;

import net.sourceforge.opencamera.realcammi.JavaImageFunctionsHDR;
import net.sourceforge.opencamera.realcammi.JavaImageFunctionsPreview;
...
import net.sourceforge.opencamera.realcammi.cameracontroller.CameraController1;
import net.sourceforge.opencamera.realcammi.cameracontroller.CameraController2;
import net.sourceforge.opencamera.realcammi.cameracontroller.CameraControllerException;
import net.sourceforge.opencamera.realcammi.cameracontroller.CameraControllerManager;
import net.sourceforge.opencamera.realcammi.cameracontroller.CameraControllerManager1;
import net.sourceforge.opencamera.realcammi.cameracontroller.CameraControllerManager2;
import net.sourceforge.opencamera.realcammi.preview.ApplicationInterface.NoFreeStorageException;
import net.sourceforge.opencamera.realcammi.preview.camerasurface.CameraSurface;
import net.sourceforge.opencamera.realcammi.preview.camerasurface.MySurfaceView;
import net.sourceforge.opencamera.realcammi.preview.camerasurface.MyTextureView;
 ...
 *  supports much of the RealCamMI logic and functionality). Communication to
...
public class Preview implements SurfaceHolder.Callback, TextureView.SurfaceTextureListener {
    ...
    private List<String> noise_reduction_modes; // n.b., this is for the Camera2 API setting, not for RealCamMI's Noise Reduction photo mode
    ...
    public void getMeasureSpec(int [] spec, int widthSpec, int heightSpec) {
        ...
        if( result % 180 != 0 ) {
            ...
            // E.g., if the device orientation is in portrait, it might still be that RealCamMI
    ...
    public void onSurfaceTextureSizeChanged(@NonNull SurfaceTexture texture, int width, int height) {
        ...
        if( camera_controller != null ) {
            ...
            if( set_preview_size && (width != preview_w || height != preview_h) ) {
                ...
                // Needed to fix problem if RealCamMI is already running, and the aspect ratio changes (e.g.,
    ...
    private class CloseCameraTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void result) {
            ...
            if( reopen ) {
                if( MyDebug.LOG )
                    Log.d(TAG, "onPostExecute, reRealCamMI");
                openCamera();
            }
            if( MyDebug.LOG )
    ...
    private void openCamera() {
        ...
        if( applicationInterface.isPreviewInBackground() ) {
            if( MyDebug.LOG )
                Log.d(TAG, "don't RealCamMI as preview in background");
            ...
            Log.d(TAG, "tried to RealCamMI while camera is still closing in background thread");
            return;
        }
        ...
        if( this.is_paused ) {
            if( MyDebug.LOG ) {
                Log.d(TAG, "don't RealCamMI as paused");
            }
            return;
        }
		...
		/*{
			...
			if( debug_count_opencamera++ == 0 ) {
				if( MyDebug.LOG )
					Log.d(TAG, "debug: don't RealCamMI yet");
				return;
			}
		}*/
        ...
        if( MyDebug.LOG ) {
            Log.d(TAG, "openCamera: total time to RealCamMI: " + (System.currentTimeMillis() - debug_time));
        }
    }
    ...
    private CameraController openCameraCore(int cameraId, String cameraIdSPhysical) {
        ...
        try {
            ...
            MyDebug.logStackTrace(TAG, "failed to RealCamMI", e);
    ...
    public CameraController.Size getOptimalPreviewSize(List<CameraController.Size> sizes) {
        ...
        Point display_size = new Point();
        {
            ...
            // (a) it would be good to not assume RealCamMI runs in landscape mode (if we ever ran in portrait mode,
    ...
    public void switchVideo(boolean during_startup, boolean change_user_pref) {
        ...
        if( is_video != old_is_video ) {
            ...
            if( !during_startup ) {
                ...
                // at the least, we need to reRealCamMI when: ( !is_video && focus_value != null && focus_value.equals("focus_mode_continuous_picture") ).
    ...
    private void takePhoto(boolean skip_autofocus, final boolean continuous_fast_burst) {
        ...
        if( autofocus_in_continuous_mode ) {
            ...
            boolean optimise_for_quality = applicationInterface.optimiseFocusForquality();
            if( optimise_for_quality ) {
                if( MyDebug.LOG )
                    Log.d(TAG, "take photo under continuous focus mode [optimise for quality]");
                takePhotoWhenFocused(continuous_fast_burst);
            }
            else {
                if( MyDebug.LOG )
                    Log.d(TAG, "call autofocus for continuous focus mode [optimise for latency]");
    ...
    public void onResume() {
        ...
        if( camera_open_state == CameraOpenState.CAMERAOPENSTATE_CLOSING ) {
            ...
            // we won't be able to open the camera, so need to RealCamMI when it's closed
            if( MyDebug.LOG )
     ...
     *  RealCamMI always runs in landscape).
    ...
    public void showToast(final ToastBoxer clear_toast, final String message, final int offset_y_dp, final boolean use_fake_toast, boolean dont_clear) {
        ...
        if( this.app_is_paused && use_fake_toast ) {
            ...
            // is useful for cases where the toast is created shortly before RealCamMI resumes, e.g., cancelling SAF
