...
public interface ApplicationInterface {
    ...
    boolean optimiseFocusForquality(); // behaviour for taking photos with continuous focus mode: if true, optimise focus for latency (take photo asap); if false, optimise for quality (don't take photo until scene is focused)
    ...
    void requestCameraPermission(); // for Android 6+: called when trying to RealCamMI, but CAMERA permission not available
    ...
    void requestStoragePermission(); // for Android 6+: called when trying to RealCamMI, but WRITE_EXTERNAL_STORAGE permission not available
