...
public class CameraController2 extends CameraController {
    ...
    // They were hand-tuned against real hardware (Ulefone Armor 25T, Xiaomi, Redmi Note 13 Pro 5G)
    ...
    private boolean want_jpeg_r = true; // test
    ...
    private float capture_result_zoom_ratio = 1.0f; // [REALCAMMI FORK] last zoom ratio from capture result, used for software crop
    ...
    // Sets the minimum allowed value for white balance. This extreme creates very cool blue tones if is image is to red/hot.
    private final static int min_white_balance_temperature_c = 1000;

    // Sets the maximum allowed value for white balance. This extreme creates very warm red/orange tones if image is to blue/cold.
            ...
            if (is_xiaomi && camera_settings.current_zoom_ratio > 1.0f) {
                try {
                    ...
                    if (originalBitmap != null) {
                        ...
                        // [REALCAMMI FORK] use the zoom ratio from the last capture result rather than
                        // camera_settings.current_zoom_ratio, which can be stale when pinch zoom is used
                        // (setZoom() is only called for discrete zoom steps, not every gesture frame).
                        // Fall back to camera_settings.current_zoom_ratio if capture result has no zoom.
                        ...
                        Log.d(TAG, "Software crop applied successfully for zoom: " + zoom);
                    }
                } catch (Exception e) {
                    Log.e(TAG, "Software crop failed", e);
                }
            }
            image.close();
                    ...
                    if( burst_type != BurstType.BURSTTYPE_FOCUS ) {
                        ...
                        /*if (supports_optical_stabilization) {
                            ...
                            try {
                                ...
                                if (slow_burst_capture_requests != null && n_burst_taken < slow_burst_capture_requests.size()) {
                                    CaptureRequest currentReq = slow_burst_capture_requests.get(n_burst_taken);
                                    if (currentReq != null) {
                                        Long exposure_time = currentReq.get(CaptureRequest.SENSOR_EXPOSURE_TIME);
    ...
    public CameraController2(Context context, int cameraId, String cameraIdSPhysical, Map<String, CameraFeaturesCache> camera_features_caches, final ErrorCallback preview_error_cb, final ErrorCallback camera_error_cb) throws CameraControllerException {
            ...
            public void onOpened(@NonNull CameraDevice cam) {
                ...
                if (first_callback) {
                    ...
                    try {
                        ...
                        String cameraIdToUse =
                                (cameraIdSPhysical != null) ? cameraIdSPhysical :
                                        (selectedPhysicalCameraId != null) ? selectedPhysicalCameraId :
                                        cameraIdS;

                        characteristics =
                                manager.getCameraCharacteristics(cameraIdToUse);

                        if (MyDebug.LOG)
                            Log.d(TAG, "successfully obtained camera characteristics");

                        characteristics_sensor_orientation =
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S
                                && !block_vendor_extensions
                                && cameraIdSPhysical == null) {

                            extension_characteristics =
                                    manager.getCameraExtensionCharacteristics(cameraIdS);

                            if (MyDebug.LOG)
                                Log.d(TAG, "successfully obtained extension characteristics");

                            camera_features_cache =
                                    camera_features_caches.get(cameraIdS);
                        }
        ...
        try {
            ...
            // Local variable to process the ID without breaking the "final" requirement
            String targetIdS = null;
            try {
                CameraControllerManager2 manager2 = new CameraControllerManager2(this.context);
                targetIdS = manager2.getCameraIdString(cameraId);
            }
            ...
            // PREVENTIVE ANTI-FREEZE FIX:
            // If the resolved ID is the virtual front camera ("4" or "5"), silently redirect
            // to ID "1" (the standard physical front camera on the Android ecosystem).
            if ("4".equals(targetIdS) || "5".equals(targetIdS)) {
                Log.e(TAG, "Safe redirect: virtual ID " + targetIdS + " changed to real front camera 1");
                targetIdS = "1";
            }

            this.cameraIdS = targetIdS;
            ...
            if( MyDebug.LOG )
                Log.d(TAG, "RealCamMI: " + idToOpen);

            cameraManager.openCamera(idToOpen, myStateCallback, handler);


            if( MyDebug.LOG )
                Log.d(TAG, "RealCamMI request complete");
        }
        catch(CameraAccessException e) {
            MyDebug.logStackTrace(TAG, "failed to RealCamMI: CameraAccessException", e);
            throw new CameraControllerException();
        }
        catch(UnsupportedOperationException e) {
            // Google Camera catches UnsupportedOperationException
            MyDebug.logStackTrace(TAG, "failed to RealCamMI: UnsupportedOperationException", e);
            throw new CameraControllerException();
        }
        catch(SecurityException e) {
            // Google Camera catches SecurityException
            MyDebug.logStackTrace(TAG, "failed to RealCamMI: SecurityException", e);
            throw new CameraControllerException();
        }
        catch(IllegalArgumentException e) {
            // have seen this from Google Play
            MyDebug.logStackTrace(TAG, "failed to RealCamMI: IllegalArgumentException", e);
            throw new CameraControllerException();
        }
            ...
            MyDebug.logStackTrace(TAG, "failed to RealCamMI: ArrayIndexOutOfBoundsException", e);
            throw new CameraControllerException();
        }
    ...
    public CameraFeatures getCameraFeatures() throws CameraControllerException {
        ...
        if (cameraIdSPhysical != null) {
            // If we have a physical camera ID, characteristics refer to the physical camera ID.
            // But for some things, we want to query the logical camera.
            ...
            try {
                logical_characteristics =
                        manager.getCameraCharacteristics(cameraIdS);

                logical_capabilities =
                        logical_characteristics.get(CameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES);

                Set<String> physicalCameras =
                        logical_characteristics.getPhysicalCameraIds();

                if (MyDebug.LOG) {
                    if (physicalCameras != null && !physicalCameras.isEmpty()) {
                        for (String id : physicalCameras) {
                            Log.d(TAG, "Physical camera found: " + id);
                        }
                    } else {
                        Log.d(TAG, "No physical cameras exposed (logical-only device or limited HAL)");
                    }
                ...
                MyDebug.logStackTrace(TAG,
                        "failed to get logical_characteristics for: " + cameraIdS, e);
                throw new CameraControllerException();
            }
        } else {
        ...
        // support fast burst - and which RealCamMI never allowed with Camera2 before 1.45).
        // Also may affect Samsung Galaxy A8(2018).
...
//      Frame duration is in nanoseconds.  Using min to be safe.
    ...
    /** Switches to a different physical camera lens (e.g. ultrawide, telephoto).
     *  Updates selectedPhysicalCameraId and closes the current capture session
     *  so it will be re-created with the new physical camera on the next preview start.
     */
    public void switchLens(String physicalId) {
        if (MyDebug.LOG)
            Log.d(TAG, "switchLens called: " + physicalId);

        selectedPhysicalCameraId = physicalId;

        try {
            closeCaptureSession();
        } catch (Exception e) {
            MyDebug.logStackTrace(TAG, "error closing capture session in switchLens", e);
        }
    }

    /**
     * Returns true if the given camera ID is NOT in the standard getCameraIdList().
     * These are the hidden sensors Xiaomi/Ulefone blocks from direct openCamera() calls.
     */
    private boolean isHiddenCameraId(CameraManager manager, String id) {
        try {
            for (String standardId : manager.getCameraIdList()) {
                if (standardId.equals(id)) return false;
            }
            return true;
        } catch (Exception e) {
            Log.e(TAG, "isHiddenCameraId failed: " + e.getMessage());
            return false;
        }
    }

    /**
     * Finds the logical camera that lists the given physical ID in getPhysicalCameraIds().
     * Falls back to "0" if not found, since that's the main back camera on all Xiaomi devices.
     */
    @RequiresApi(api = Build.VERSION_CODES.P)
    private String findLogicalParentForPhysicalId(CameraManager manager, String physicalId) {
        try {
            for (String logicalId : manager.getCameraIdList()) {
                try {
                    ...
                    if (physicalIds != null && physicalIds.contains(physicalId)) {
                        Log.e(TAG,
                                "Found logical parent: " + physicalId + " -> " + logicalId);
                        return logicalId;
                    }
            ...
            Log.e(TAG, "findLogicalParent failed: " + e.getMessage());
        }

        Log.e(TAG, "No logical parent found for " + physicalId);
        return null;
    }
    ...
    private List<OutputConfiguration> createOutputConfigurationList(List<Surface> surfaces, Surface preview_surface) {
        ...
        for(Surface surface : surfaces) {
            ...
            // Route output to the correct physical sensor.
            String physIdForOutput = selectedPhysicalCameraId;
            if( physIdForOutput != null ) {
                try {
                    config.setPhysicalCameraId(physIdForOutput);
                    Log.e(TAG, "setPhysicalCameraId SUCCESS: " + physIdForOutput);
                } catch( Exception e ) {
                    Log.e(TAG, "setPhysicalCameraId FAILED for ID " + physIdForOutput + ": " + e.getMessage());
                }
            }
    ...
    public void autoFocus(final AutoFocusCallback cb, boolean capture_follows_autofocus_hint) {
        ...
        synchronized( background_camera_lock ) {
            ...
            if( focus_mode == null ) {
                ...
                // (unclear if RealCamMI needs this, but just to be safe and consistent between camera APIs)
                if( MyDebug.LOG )
    ...
    private void takePictureAfterPrecapture() {
        ...
        synchronized( background_camera_lock ) {
            ...
            try {
                ...
                } else if (camera_settings.has_iso && !is_xiaomi) {
                    templateType = CameraDevice.TEMPLATE_MANUAL;
                }

                stillBuilder = camera.createCaptureRequest(templateType);
                stillBuilder.setTag(new RequestTagObject(RequestTagType.CAPTURE));
                camera_settings.setupBuilder(stillBuilder, true);

                // Redmi Note 13 Pro 5G (and other Xiaomi devices): ensure zoom is applied
                // to the still capture request as well as the preview.
                camera_settings.setCropRegion(stillBuilder);
                camera_settings.setControlZoomRatio(stillBuilder);
    ...
    public static List<Float> setupFocusBracketingDistances(float source, float target, int count) {
        ...
        final float max_focus_bracket_distance_c = 0.15f; // change from 10m to 15m
    ...
    public void initVideoRecorderPostPrepare(MediaRecorder video_recorder, boolean want_photo_video_recording)
        ...
        // [FIX] The previous implementation here incorrectly called manager.openCamera() with a
        // do-nothing StateCallback in an attempt to work around Xiaomi sub-lens ID issues.
        // This was fundamentally broken: it opened a second, disconnected CameraDevice object and
        // immediately abandoned it, leaving the camera subsystem in an inconsistent state. On many
        // devices (including Xiaomi/Redmi) this caused autofocus to stop responding, the preview to
        // freeze, and sometimes a full camera crash on the first video recording attempt.
        //
        // The correct behaviour is to attach the prepared MediaRecorder surface to a new
        // Camera2 capture session before MediaRecorder.start() is called. Without this, the
        // recorder starts successfully but receives 0 video frames, and stop() fails with -1007.
        //
        // No second openCamera() call is needed or correct here.
        //
        // Physical-camera / sub-lens routing for Xiaomi is already handled correctly at session-
        // creation time via selectedPhysicalCameraId and createOutputConfigurationList(), so there
        // is nothing special to do here for those devices.
        if( camera == null ) {
            Log.e(TAG, "no camera");
            throw new CameraControllerException();
        }
        ...
        try {
            video_recorder_surface = video_recorder.getSurface();
            Log.e(TAG, "initVideoRecorderPostPrepare: got MediaRecorder surface: " + video_recorder_surface);
            if( MyDebug.LOG )
                Log.d(TAG, "video_recorder_surface: " + video_recorder_surface);
            Log.e(TAG, "initVideoRecorderPostPrepare: rebuilding capture session with recorder surface");
            createCaptureSession(true, null, null, video_recorder, want_photo_video_recording);
            Log.e(TAG, "initVideoRecorderPostPrepare: capture session ready for MediaRecorder.start()");
        }
        catch(IllegalStateException e) {
            // getSurface() can throw if the MediaRecorder is in the wrong state
            MyDebug.logStackTrace(TAG, "failed to get video recorder surface", e);
            throw new CameraControllerException();
        }
    }
    ...
    */
    ...
    private class MyCaptureCallback extends CameraCaptureSession.CaptureCallback {
        ...
        private void updateCachedCaptureResult(CaptureResult result) {
            ...
            // [REALCAMMI FORK] read zoom ratio from capture result for use in software crop
            // (camera_settings.current_zoom_ratio can be stale when pinch zoom is used)
            if( Build.VERSION.SDK_INT >= Build.VERSION_CODES.R ) {
                Float zoom_ratio = result.get(CaptureResult.CONTROL_ZOOM_RATIO);
                if( zoom_ratio != null ) {
                    capture_result_zoom_ratio = zoom_ratio;
                }
            }
